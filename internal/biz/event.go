...
// Event 是 API 发布到消息系统的业务 DTO。业务层不依赖具体 Kafka 客户端，
// 因而单元测试和未来替换消息基础设施时都无需构造 franz-go 类型。
type Event struct {
	Topic   string
	Key     []byte
	Payload []byte
...
// EventPublisher 定义业务层真正需要的消息发布能力。具体 Kafka Producer 在
// data 层实现该接口，依赖方向仍然是 infrastructure -> business contract。
type EventPublisher interface {
	Publish(context.Context, Event) error
}

// PublishAccepted 是统一的异步消息发布结果。ResourceID 是调用方可立即
// 返回给客户端的业务 ID；它不表示 Consumer 已完成处理。
...
// MessagePublisher 是所有业务共用的消息发布器。body 可以是任意可被 JSON
// 编码的值，不要求实现统一接口；Topic 和 key 是路由元数据，由调用方传入。
...
// EventService 校验 HTTP 请求选择的 Topic，并协调可靠发布。allowedTopics 是
// 明确白名单，避免调用方借助公共 API 向任意内部 Topic 写入消息。
