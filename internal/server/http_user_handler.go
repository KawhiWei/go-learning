...
// Create 处理 POST /v1/users。API 不再直接调用 UserService.CreateUser 写库，
// 而是把创建消息发布到 user-events；数据库写入发生在独立 work Consumer。
...
func (h UserHTTPHandler) Create(ctx context.Context, c *app.RequestContext) {
	...
	// UserCreateMessage 只是生产者与消费者约定的消息体，不需要实现公共
	// 接口。统一 Publisher 负责 JSON 编码、Topic 白名单和底层发送。
	message, err := biz.NewUserCreateMessage(req.Name, req.Email)
	if err != nil {
		writeServiceError(c, err)
		return
	}
	// Topic 决定消息去往哪个业务队列；相同 user_id 使用相同 key，可以让
	// Kafka 把同一用户的消息稳定路由到同一 partition 并保持处理顺序。
